...
function NavItemWithGlitch({
  ...
  return (
    <a
      href={item.path}
      onClick={onClick}
      className={`${isMobile ? "block" : ""} px-3 py-2 rounded-md ${isMobile ? "text-base" : "text-lg"} font-mono transition-all duration-300 flex items-center cursor-pointer ${pixelifySans.className} ${
    ...
    >
      <div ref={glitch.ref} className="flex items-center">
        {isActive && (
          <span
            className={`w-2 h-2 ${isMobile ? "rounded-full" : ""} bg-green-500 mr-2`}
          />
        )}
        {item.name}
      </div>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navItems.map((item) => (
              <a
                ...
                href={item.path}
                onClick={(e) => handleNavClick(e, item.path)}
                className={`px-3 py-2 rounded-md text-base font-medium flex items-center cursor-pointer ${pixelifySans.className} ${
